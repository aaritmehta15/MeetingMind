...
import { Layers, Database, RefreshCw, Send, FileText, CheckCircle2, Quote, ExternalLink } from 'lucide-react';

export default function CorpusStudio({ examples, provider }) {
  const [question, setQuestion] = useState('what decisions were made about the sales strategy across all meetings?');
  const [loading, setLoading] = useState(false);
  const [indexing, setIndexing] = useState(false);
  const [result, setResult] = useState(null);

  const sampleCorpusQueries = [
  ...
  const handleRebuildCorpus = async () => {
    ...
    try {
      ...
      alert(`Corpus indexed successfully! ${data.total_chunks} chunks indexed across ${data.num_meetings} meetings.`);
    } catch (err) {
    ...
    <div style={{ padding: '24px 28px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '4px' }}>
            Index your entire meeting library into a unified FAISS vector store to synthesize decisions and commitments across multiple transcripts with document citations.
          </p>
        </div>
        ...
        <button onClick={handleRebuildCorpus} disabled={indexing} className="btn btn-secondary btn-sm">
          ...
          <span>{indexing ? 'Indexing Corpus...' : 'Re-index Meeting Files'}</span>
        </button>
      </div>

      {/* Indexed Meetings Pills */}
      <div className="glass-panel" style={{ padding: '16px 20px', marginBottom: '24px' }}>
        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-dim)', textTransform: 'uppercase', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Database size={13} />
          <span>Active Corpus Documents ({examples.length} Indexed Meetings)</span>
        </div>
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          {examples.map((ex) => (
            ...
            >
              ...
              <div>
                <div style={{ fontSize: '0.825rem', fontWeight: 600, color: 'var(--text-main)' }}>{ex.filename}</div>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{ex.turn_count} turns indexed</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="glass-panel" style={{ padding: '20px', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.75rem', color: 'var(--text-dim)', fontWeight: 600 }}>Try queries:</span>
          {sampleCorpusQueries.map((sq, idx) => (
            <button
              key={idx}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1.4fr) minmax(320px, 1fr)', gap: '24px' }}>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
              <span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#a5b4fc', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Cross-Meeting Synthesized Answer
              </span>
            ...
            <div style={{ marginTop: '16px', fontSize: '0.725rem', color: 'var(--text-dim)', borderTop: '1px solid var(--border-subtle)', paddingTop: '10px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>
              Referenced Source Excerpts ({result.sources.length})
            </div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-dim)' }}>
                    Relevance: <strong style={{ color: '#67e8f9' }}>{src.score}</strong>
                  </span>
                </div>
                <div style={{
                  fontSize: '0.775rem',
